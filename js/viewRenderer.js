...
const ViewRenderer = {
  ...
  getHrMgmtView() {
    ...
    const storeManagers = staffList.filter(s => s.role.includes('Quản lý'));
    const accountants = staffList.filter(s => s.role.includes('Kế toán'));
    const employees = staffList.filter(s => s.role.includes('Nhân viên'));

    return `
      <div class="subpage-header">
        <div>
          <div class="subpage-breadcrumb">Doanh nghiệp / <strong>Quản trị Nhân lực</strong></div>
          <h1 class="subpage-title">Quản Trị Nhân Lực & Phân Quyền Theo Vai Trò</h1>
          <p style="font-size:13px; color:var(--text-muted); margin-top:2px;">Phân quyền theo các vai trò: Quản lý cửa hàng, Kế toán, Nhân viên.</p>
        </div>
        <div style="display:flex; gap:10px;">
          <button class="btn-primary" onclick="openAddStaffModalBRD()">+ Thêm Nhân Viên Mới</button>
        </div>
      </div>

      <!-- PHẦN TỔNG QUAN PHÂN QUYỀN VAI TRÒ (3 KPI STATS BAR) -->
      <div class="enterprise-cards-grid" style="margin-bottom:20px; display:grid; grid-template-columns: repeat(3, 1fr); gap:16px;">
        <div class="table-card" style="padding:16px; border-left:4px solid #16A34A;">
          <div style="font-size:12px; color:var(--text-muted); font-weight:600; text-transform:uppercase;">👑 QUẢN LÝ CỬA HÀNG</div>
          <div style="font-size:24px; font-weight:800; color:#16A34A; margin-top:4px;">${storeManagers.length} <span style="font-size:13px; font-weight:500; color:var(--text-muted);">nhân sự</span></div>
          <div style="font-size:11.5px; color:#15803D; margin-top:4px;">Toàn quyền vận hành & duyệt đơn chi nhánh</div>
        </div>

        <div class="table-card" style="padding:16px; border-left:4px solid #0284C7;">
          <div style="font-size:12px; color:var(--text-muted); font-weight:600; text-transform:uppercase;">📊 KẾ TOÁN (ACCOUNTANT)</div>
          <div style="font-size:24px; font-weight:800; color:#0284C7; margin-top:4px;">${accountants.length} <span style="font-size:13px; font-weight:500; color:var(--text-muted);">nhân sự</span></div>
          <div style="font-size:11.5px; color:#0369A1; margin-top:4px;">Quyền đối soát doanh số, hóa đơn & thu chi</div>
        </div>

        <div class="table-card" style="padding:16px; border-left:4px solid #EA580C;">
          <div style="font-size:12px; color:var(--text-muted); font-weight:600; text-transform:uppercase;">👤 NHÂN VIÊN (STAFF / POS)</div>
          <div style="font-size:24px; font-weight:800; color:#EA580C; margin-top:4px;">${employees.length} <span style="font-size:13px; font-weight:500; color:var(--text-muted);">nhân sự</span></div>
          <div style="font-size:11.5px; color:#C2410C; margin-top:4px;">Quyền thu ngân bán hàng POS & mở két tiền</div>
        </div>
      </div>

      <!-- BẢNG DANH SÁCH TÀI KHOẢN & NHÂN VIÊN TRONG HỆ THỐNG -->
      <div class="table-card" style="margin-bottom:24px;">
        <div class="table-header" style="padding:16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #F1F5F9;">
          <h3 style="font-size:15px; font-weight:700; color:var(--text-main); margin:0;">Danh Sách Tài Khoản & Phân Quyền Vai Trò</h3>
          ...
          <table class="portal-table">
            <thead>
              <tr style="background:#F8FAFC;">
                ...
                <th style="font-size:11.5px; font-weight:700; color:#64748B; padding:10px 16px; text-align:center;">THAO TÁC</th>
              </tr>
            </thead>
            <tbody id="tbodyStaffOverview">
              ${staffList.map(s => `
                <tr>
                  ...
                  <td style="padding:12px 16px; font-family:monospace; color:#334155;">${s.mobile}</td>
                  <td style="padding:12px 16px;">
                    ${s.role.includes('Quản lý') ? `
                      <span style="background:#DCFCE7; color:#15803D; padding:3px 10px; border-radius:12px; font-size:11.5px; font-weight:700; display:inline-block;">👑 Quản lý cửa hàng</span>
                    ` : s.role.includes('Kế toán') ? `
                      <span style="background:#E0F2FE; color:#0369A1; padding:3px 10px; border-radius:12px; font-size:11.5px; font-weight:700; display:inline-block;">📊 Kế toán</span>
                    ` : `
                      <span style="background:#FFEDD5; color:#C2410C; padding:3px 10px; border-radius:12px; font-size:11.5px; font-weight:700; display:inline-block;">👤 Nhân viên</span>
                    `}
                  </td>
                  <td style="padding:12px 16px; color:#334155;">${(s.branches || []).join(', ')}</td>
                  <td style="padding:12px 16px;">
                    <span class="status-badge ${s.statusClass}">${s.statusText}</span>
                  </td>
                  <td style="padding:12px 16px; text-align:center;">
                    <a href="javascript:void(0)" onclick="openEditStaffModalBRD('${s.id}')" style="color:#0284C7; font-weight:700; font-size:12.5px; text-decoration:none; padding:4px 8px; border:1px solid #BFDBFE; border-radius:4px; background:#EFF6FF;">Sửa Quyền</a>
                  </td>
                </tr>
              `).join('')}
            </tbody>
  ...
  getStatementV11View() {
    ...
    return `
      <div class="subpage-header">
        <div>
          <div class="subpage-breadcrumb">Đối soát / <strong>Sao Kê 1.1</strong></div>
      ...
      <div class="table-card" style="margin-bottom:20px;">
        ...
        <form id="v11StatementFilterForm" onsubmit="return false;">
          <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px;">
            ...
            <!-- 2. Khoảng thời gian (Ngày hạch toán) -->
            <div class="form-group-field">
  ...
  getReconcileReportV1View() {
    ...
    const dailyTxns = MockData.getDailyTransactionsV1Data ? MockData.getDailyTransactionsV1Data() : [];
    ...
    return `
      ...
      <!-- ==================================================== -->
      <!-- TAB 1: BÁO CÁO TỔNG QUAN (CÓ DASHBOARD + CÁC GIAO DỊCH TRONG NGÀY) -->
      <!-- ==================================================== -->
      <div id="v1TabWithDashContent" style="display:block;">
        ...
        <div class="table-card" style="margin-bottom:20px;">
          <div style="font-size:15px; font-weight:700; margin-bottom:14px; color:var(--text-main); display:flex; align-items:center; gap:8px;">
            <i data-lucide="search" style="width:16px; height:16px; color:var(--color-primary);"></i> Bộ Lọc Tra Cứu Giao Dịch
          </div>
          <form onsubmit="return false;">
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:16px; padding-top:12px; border-top:1px dashed var(--border-color);">
              <button type="button" class="btn-secondary" onclick="showToast('Đã làm lại bộ lọc')">Đặt lại</button>
              <button type="button" class="btn-primary" onclick="showToast('Đang tra cứu các giao dịch trong ngày...')"><i data-lucide="search" style="width:14px; height:14px; margin-right:4px;"></i> Tra cứu</button>
            </div>
          </form>
        </div>

        <!-- BẢNG CHI TIẾT CÁC GIAO DỊCH TRONG NGÀY HÔM ĐÓ -->
        <div class="table-card">
          <div class="table-header" style="padding:14px 16px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border-color); background:var(--bg-card-subtle, #F8FAFC);">
            <div>
              <span style="font-weight:800; font-size:14px; color:var(--text-main);">Chi tiết Các Giao dịch trong ngày hôm đó (26/08/2026)</span>
              <span style="font-size:12px; color:var(--text-muted); margin-left:8px;">(Tổng <strong>${dailyTxns.length}</strong> giao dịch)</span>
            </div>
            <div style="display:flex; align-items:center; gap:16px;">
              <button class="btn-primary" style="font-size:12px; padding:6px 12px;" onclick="showToast('Đang xuất danh sách giao dịch trong ngày...')"><i data-lucide="download" style="width:14px; height:14px; margin-right:4px;"></i> Xuất danh sách giao dịch</button>
            </div>
          </div>
            ...
            <table class="portal-table">
              <thead>
                <tr>
                  ...
                  <th style="white-space:nowrap;">Thời gian phát sinh</th>
                  <th style="white-space:nowrap;">Phương thức thanh toán</th>
                  <th style="white-space:nowrap;">Cửa hàng</th>
                  <th style="white-space:nowrap; text-align:right;">Số tiền gốc</th>
                  <th style="white-space:nowrap; text-align:right;">Phí giao dịch</th>
                  <th style="white-space:nowrap; text-align:right;">Số tiền hoàn/hủy</th>
                  <th style="white-space:nowrap; text-align:right;">Thực nhận</th>
                  <th style="white-space:nowrap; text-align:center;">Trạng thái</th>
                </tr>
              </thead>
              <tbody>
                ${dailyTxns.map(t => `
                  <tr>
                    <td style="text-align:center; font-weight:600;">${t.stt}</td>
                    <td style="font-weight:700; color:#0284C7;">${t.txnCode}</td>
                    <td style="font-size:12px; color:var(--text-muted);">${t.refCode}</td>
                    <td style="font-size:12px; color:var(--text-muted);">${t.approveCode}</td>
                    <td style="font-size:12.5px;">${t.txnTime}</td>
                    <td>
                      <span style="background:#F0FDF4; color:#16A34A; border:1px solid #BBF7D0; padding:2px 8px; border-radius:4px; font-size:11.5px; font-weight:700;">
                        ${t.payMethod}
                      </span>
                    </td>
                    <td style="font-size:12.5px;">${t.storeName}</td>
                    <td style="text-align:right; font-weight:700; color:#0F172A;">${t.originalAmount}</td>
                    <td style="text-align:right; color:#EF4444; font-size:12.5px;">${t.fee}</td>
                    <td style="text-align:right; color:var(--text-muted); font-size:12.5px;">${t.refundAmount}</td>
                    <td style="text-align:right; font-weight:800; color:#16A34A; font-size:13.5px;">${t.netAmount}</td>
                    <td style="text-align:center;">
      ...
      <!-- ==================================================== -->
      <!-- TAB 2: BẢNG DANH SÁCH (KHÔNG DASHBOARD - LỌC THEO MÃ THANH TOÁN & CỬA HÀNG) -->
      <!-- ==================================================== -->
      <div id="v1TabNoDashContent" style="display:none;">
        <!-- MỤC TÌM KIẾM BÁO CÁO (LỌC THEO MÃ THANH TOÁN & CỬA HÀNG) -->
        <div class="table-card" style="margin-bottom:20px;">
          <div style="font-size:15px; font-weight:700; margin-bottom:14px; color:var(--text-main); display:flex; align-items:center; gap:8px;">
            <i data-lucide="search" style="width:16px; height:16px; color:var(--color-primary);"></i> Bộ Lọc Báo Cáo Đối Soát
          </div>
          <form onsubmit="return false;">
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px;">
              ...
              <div class="form-group-field">
                <label>Mã thanh toán</label>
                <input type="text" id="filterV1CodeInput" placeholder="Ví dụ: R_16566_22072025_131523_772">
              </div>

              <!-- Cửa hàng -->
              <div class="form-group-field">
                <label>Cửa hàng</label>
                <select id="filterV1StoreNoDash">
                  <option value="all">Tất cả cửa hàng</option>
                  <option value="ST-Q1-001">Chi nhánh Quận 1 - HCM</option>
                  <option value="ST-HK-002">Chi nhánh Hoàn Kiếm - Hà Nội</option>
                  <option value="ST-HC-003">Chi nhánh Hải Châu - Đà Nẵng</option>
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:16px; padding-top:12px; border-top:1px dashed var(--border-color);">
              <button type="button" class="btn-secondary" onclick="showToast('Đã làm lại bộ lọc')">Đặt lại</button>
              <button type="button" class="btn-primary" onclick="showToast('Đang tìm kiếm theo Mã thanh toán & Cửa hàng...')"><i data-lucide="search" style="width:14px; height:14px; margin-right:4px;"></i> Tra cứu</button>
            </div>
          </form>
        </div>
